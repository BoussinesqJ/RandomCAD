...
RandomCAD 是一个专业的随机骨料颗粒生成器，用于混凝土结构建模，也可用于采空区邻位充填的二维建模。它支持在 AutoCAD 和中望CAD 中生成随机分布的骨料颗粒，并提供完整的参数配置和可视化功能。
...
### 🚀 选项 A：使用独立可执行文件 (.exe) [推荐]

如果您不想配置 Python 环境，可以直接编译并运行打包好的独立程序：
1. **自动编译**：在项目根目录下运行 `python packaging/build_exe.py`。
2. **直接运行**：在 `packaging/dist/` 文件夹下双击 `RandomCAD.exe` 即可直接启动运行。
*(详细打包与体积优化指南请参考 [打包说明文档](packaging/README.md))*
...
- **✨ 新增本地自适应预览画布**：
  - 基于 `QGraphicsView` 编写了独立的 2D 画布组件，支持与 CAD 双通道实时渲染，并在未连接 CAD 时优雅切换为本地独立预览。
  - 新增“同步绘制到 CAD”功能，支持本地满意后一键同步导入。
- **✨ 支持单组多形状级配**：
  - 各粒径分组中支持同时勾选多边形、圆形、椭圆形，并按照权重比例随机混合生成骨料。
- **🚀 GPU 碰撞检测加速与优化**：
  - 成功集成了 PyTorch CUDA (12.1) 加速算法，在大量骨料生成时利用 GPU 并行计算碰撞干涉。
  - 界面新增了 GPU 开关，并在生成期间动态管理其交互状态。
- **📊 性能基准测试工具**：
  - 编写并完善了 `benchmark_gpu.py` 性能测试脚本，实测了 RTX 2050 显卡与 CPU 模式的耗时，提供了最优选型建议。
- **📦 独立可执行程序打包 (.exe)**：
  - 新增 `packaging/` 打包目录，支持生成单文件/解压文件夹格式的独立运行版。
  - 内置 `comtypes` 缓存目录重定向及系统 site-packages 动态混合加载技术，轻量版 `.exe` 亦可动态调用外部 PyTorch 加速。
- **📝 文档与规范建设**：
  - 补充了 MIT 开源许可证及 BibTeX 学术引用说明。
  - 更新项目路线图，扩展定位至**采空区邻位充填二维细观建模**。
